...
# Capturar SIGUSR1 per mostrar el progrés de la còpia
trap "echo 'Progrés de la còpia: $(tail -n 10 /tmp/tar_progress.log)'" SIGUSR1
...
( tar czpvf "${fitxer_copia}" "${@}" > /tmp/tar_progress.log 2>&1 & echo $! > /tmp/tar_backup.pid; sleep 10 ) &
